...
public class XMLImporter {
	...
	private void updateSequenceInSequence (SequenceDocument from, SequenceDocument to, String classname, String extended) {
		...
		if (null!= to) {
			...
			System.err.println("no no no no");
		}
	}	
	...
	public static void main(String[] args) {
		...
		String dbName = "NAME DEL DB";
		
		importer.loadTokens(dbName + "/doc", "xxx", false);
		importer.loadTokens(dbName + "/doc", "yyy", false);
		importer.loadPericopes(dbName + "/doc", "xxx", false);
		importer.loadPericopes(dbName + "/doc", "yyy", false);
		...
		try {
			...
			greekCharBuffer = Utils.fromFile ("[...]/resources/greek.txt");
			//arabicCharBuffer = Utils.fromFile ("WebContent/resources/arabic.txt");
			arabicCharBuffer = Utils.fromFile ("[...]/arabic.txt");
			...
			for(Map.Entry<String, Resource> entry : importer.getResources().entrySet()){
				...
				importer.updateTokenSequence(converter.getArabicTokenSequence(), importer.getTokensSequenceDocument("xxx"), "yyyy");
				importer.updateTokenSequence(converter.getGreekTokenSequence(), importer.getTokensSequenceDocument("xxx"), "yyyy");
				System.err.print(" udpated tokens => ");
				 */
				importer.updatePericopeSequence(converter.getArabicPericopeSequence(), importer.getPericopesSequenceDocument("xxx"), "yyyy");
				importer.updatePericopeSequence(converter.getGreekPericopeSequence(), importer.getPericopesSequenceDocument("xxx"), "yyyy");
			...
			importer.createTokenSequenceFromList(importer.getArabicTS(),  importer.getTokensSequenceDocument("xxx"),  "yyyy");
			importer.createTokenSequenceFromList(importer.getGreekTS(),  importer.getTokensSequenceDocument("xxx"),  "yyyy");
			
			importer.saveTokens (dbName + "/doc", "xxx");
			importer.saveTokens (dbName + "/doc", "yyy");

			importer.savePericopes(dbName + "/doc", "xxx");
			importer.savePericopes(dbName + "/doc", "yyy");

			importer.saveLinks(dbName + "/link" );

			//String greekTokenList = importer.getTokensSequenceDocument("xxx").toString();
			//String greekPericopeList = importer.getPericopesSequenceDocument("xxx").toString();
