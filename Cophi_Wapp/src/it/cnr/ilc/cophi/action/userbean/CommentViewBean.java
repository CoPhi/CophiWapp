...
public class CommentViewBean implements Serializable {
	...
	public List<Comment> getLinkComments(){
		...
		LinkViewEntity lve = getSelectedViewLink();
		if (null != lve) {
	...
	public String deleteComment() {
		...
		if(success) {
			...
			resetForm();
			repositoryBean.reloadAllComments(getSelectedViewLink().getLink().getId());
		} else {
	...
	public String loadComment () {
		...
		if (null != selectedComment.getArabicSelectedBound()) {
			try {
				setArabicSelectedText(Utils.substring(selectedComment.getArabicSelectedBound(), selectedViewLink.getArabicPericopeText()));
			} catch (java.lang.StringIndexOutOfBoundsException e) {
				
				System.err.println(e.toString() + " Error in "+ selectedComment.getCommentId() +", arabic substring, boundaries are outside " + selectedComment.getGreekSelectedBound().getStart() + ":" + selectedComment.getGreekSelectedBound().getEnd() + ", over " + selectedViewLink.getArabicPericopeText().length());
				
			}
		}
		if (null != selectedComment.getGreekSelectedBound()) {
			try {
				setGreekSelectedText(Utils.substring(selectedComment.getGreekSelectedBound(), selectedViewLink.getGreekPericopeText()));
			} catch (java.lang.StringIndexOutOfBoundsException e) {
				System.err.println(e.toString() + " Error in "+ selectedComment.getCommentId() +", greek substring, boundaries are outside " + selectedComment.getGreekSelectedBound().getStart() + ":" + selectedComment.getGreekSelectedBound().getEnd() + ", over " + selectedViewLink.getGreekPericopeText().length());
			}
		}
